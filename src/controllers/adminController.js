...
const { products, writePeliculasJson, generos } = require("../database/index");let controller = {    inicio: (req, res) => {
    ...
    eliminar: (req, res) => {
        let productId = +req.params.id;        products.forEach(product => {
...
/*const { products, writePeliculasJson, generos } = require("../database/index");let controller = {    inicio: (req, res) => {
    ...
    actualizar: (req, res) => {
        const { name, category, age, price, description, reparto, direction, duration } = req.body        products.forEach(product => {
            if (product.id === +req.params.id) {
                product.name = name.trim(),
                    prduct.price = price,
                    product.description = description.trim(),
    ...
    eliminar: (req, res) => {
     ...
     agregar: (req, res) => {
        const { name, category, age, price, description, reparto, duration, direction } = req.body        let lastId = 1;        products.forEach(product => {
            if (product.id > lasId) {
                lasId = products.id
            }
        });        let newProducts = {
